...
// Interceptor: Dynamically attach JWT token to every outgoing request
// This prevents header race conditions that trigger CORS preflight errors.
...
function App() {
  ...
  // Synchronize localStorage whenever local token state changes
  useEffect(() => {
    if (token) {
      ...
      try {
        ...
        setLoadError(
          err.response?.data?.message ||
            "Could not reach the backend API. Is the server running on " +
              API_BASE +
              "?",
        );
      } finally {
      ...
      >
        {dataLoading || !settings ? (
          <Route
            ...
            element={
              <div
                style={{ color: "white", padding: "40px", textAlign: "center" }}
              >
                {loadError ? (
                  <>
